...
public class EmailVerificationService implements EmailVerification {
    ...
    public void resend(String email) {
        ...
        auditPublisher.afterCommit(SecurityAuditEvent.of(SecurityAuditAction.EMAIL_VERIFICATION_REQUEST,
    ...
    public void confirm(String rawToken) {
        ...
        UUID userId = oneTimeTokenRepository.findUserIdByTokenHash(tokenHash).orElseThrow(this::invalidVerificationToken);
        User user = userRepository.findByIdForUpdate(userId).orElseThrow(this::invalidVerificationToken);
        OneTimeToken token = oneTimeTokenRepository.findByTokenHashForUpdate(tokenHash).orElseThrow(this::invalidVerificationToken);
        Instant now = clock.instant();
        if (!token.getUser().getId().equals(userId) || token.getPurpose() != TokenPurpose.VERIFY_EMAIL
                || !token.isUsableAt(now) || user.getStatus() != AccountStatus.PENDING_VERIFICATION) {
            throw invalidVerificationToken();
        }
        token.consume(now);
        oneTimeTokenRepository.invalidateOtherActiveTokens(userId, TokenPurpose.VERIFY_EMAIL, token.getId(), now);
        user.verifyEmail(now);
        auditPublisher.afterCommit(SecurityAuditEvent.of(SecurityAuditAction.EMAIL_VERIFICATION_CONFIRM,
                SecurityAuditOutcome.SUCCESS, SecurityAuditReason.NONE, userId, null));
    }

    private void issueVerificationForLockedUser(User user) {
        if (user.isEmailVerified() || user.getStatus() != AccountStatus.PENDING_VERIFICATION) return;
        Instant now = clock.instant();
        ...
        OneTimeToken token = oneTimeTokenRepository.save(new OneTimeToken(user, TokenPurpose.VERIFY_EMAIL,
        ...
        emailDelivery.enqueueVerification(issuanceId, user.getEmail(), generatedToken.value(), expiresAt, now);
    }

    private BadRequestException invalidVerificationToken() {
        auditPublisher.now(SecurityAuditEvent.of(SecurityAuditAction.EMAIL_VERIFICATION_CONFIRM,
                SecurityAuditOutcome.FAILURE, SecurityAuditReason.INVALID_VERIFICATION_TOKEN, null, null));
        return new BadRequestException(ProblemCode.INVALID_VERIFICATION_TOKEN,
